...
đã khớp các nhóm yêu cầu chính. Nhánh hiện tại chỉ giữ NIC/PCAP runtime path,
tách pipeline thành các module rõ ràng và hoàn thiện CLI/dashboard realtime.

## 1. Mã nguồn và ứng dụng

| Yêu cầu docx | Trạng thái | Artifact hiện tại |
|---|---|---|
| Data plane nhận packet từ NIC hoặc PCAP | Đạt | `src/port.c`, `src/dispatcher.c`, ethdev mode, PCAP PMD qua `--vdev net_pcap0,rx_pcap=...` |
...
| `src/port.c` | Configure ethdev/PCAP PMD port, RX/TX queues and mbuf pool |
| `src/dispatcher.c` | RX burst, parse, canonical worker selection, ring enqueue |
...
| Performance benchmarks và kết quả | `reports/BENCHMARK.md`, `reports/e2e_benchmark_results.csv` |
| Throughput PPS | `pps` trong CSV và bảng E2E |
